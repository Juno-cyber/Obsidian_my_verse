    ...
    if (settingsKey !== null) {
      setting.addToggle(
        (toggle) => toggle.setTooltip("重载以生效").setValue(this.settingsManager.getSetting(settingsKey)).onChange(async (value) => {
          ...
          new import_obsidian2.Notice("重新加载 Obsidian 以应用更改。");
        })
      );
    }
    return setting;
...
var PresentationCanvasExtension = class extends CanvasExtension {
  ...
  async startPresentation(canvas, tryContinue = false) {
    if (!tryContinue || this.visitedNodeIds.length === 0) {
      ...
      if (!startNode2) {
        new import_obsidian12.Notice("未找到起始节点。请通过弹出菜单将一个节点标记为起始节点。");
        return;
      }
...
var ExportCanvasExtension = class extends CanvasExtension {
  ...
  async showExportImageSettingsModal(canvas, nodesToExport) {
    ...
    let svg = false;
    new import_obsidian17.Setting(modal.contentEl).setName("导出文件格式").setDesc("选择将画布导出为的文件格式。").addDropdown(
    ...
    pixelRatioSetting = new import_obsidian17.Setting(modal.contentEl).setName("像素比例").setDesc("更高的像素比例会生成更高分辨率的图像，但文件体积也会更大。").addSlider(
    ...
    noFontExportSetting = new import_obsidian17.Setting(modal.contentEl).setName("跳过字体导出").setDesc("这将不会在导出的 SVG 中包含字体。这会使 SVG 文件更小。").addToggle(
      (toggle) => toggle.setValue(noFontExport).onChange((value) => noFontExport = value)
    );
    let watermark = false;
    new import_obsidian17.Setting(modal.contentEl).setName("显示徽标").setDesc("这将在左下角添加一个 Obsidian + 高级画布 的徽标。").addToggle(
      (toggle) => toggle.setValue(watermark).onChange((value) => watermark = value)
    );
    let garbledText = false;
    new import_obsidian17.Setting(modal.contentEl).setName("隐私模式").setDesc("这将隐藏画布上的所有文本。").addToggle(
      (toggle) => toggle.setValue(garbledText).onChange((value) => garbledText = value)
    );
    let transparentBackground = false;
    transparentBackgroundSetting = new import_obsidian17.Setting(modal.contentEl).setName("透明背景").setDesc("这将使图像的背景变为透明。").addToggle(
  ...
  async exportImage(canvas, nodesToExport, svg, pixelRatioFactor, noFontExport, watermark, garbledText, transparentBackground) {
    ...
    new import_obsidian17.Notice("导出画布。请稍候...");
